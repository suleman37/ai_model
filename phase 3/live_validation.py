...
def main():
    ...
    try:
        if not os.path.exists(MODEL_PT):
            print(f"Error loading models: segmentation model not found at '{MODEL_PT}'")
            return

        seg_model = YOLO(MODEL_PT)
        if os.path.exists(MODEL_TFLITE):
            detect_model = YOLO(MODEL_TFLITE, task='detect')
            print(f"✓ Detection model loaded from {MODEL_TFLITE}")
        else:
            detect_model = seg_model
            print(f"⚠ Detection model not found at '{MODEL_TFLITE}'. Falling back to {MODEL_PT}")
